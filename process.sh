...
python src/process.py output/surma.ttl output/surma_processed.ttl --arpa_pnr $ARPA_URL/pnr_municipality &&
cat data/surma_additions.ttl >> output/surma_processed.ttl &&

echo "Linking ranks" &&
python src/linker.py ranks output/surma_processed.ttl output/rank_links.ttl --endpoint $WARSA_ENDPOINT_URL/sparql --logfile output/logs/linker.log &&

echo "Linking persons" &&
python src/sotasampo_helpers/arpa.py persons join output/surma_processed.ttl output/person_candidates_combined.ttl http://candidates/person $WARSA_ENDPOINT_URL/sparql --prop http://ldf.fi/schema/warsa/family_name -n -w 3 -r 3 &&
...
python src/tasks.py link_units output/surma_wo_units.ttl output/surma_units.ttl --endpoint $WARSA_ENDPOINT_URL/sparql --arpa_unit $ARPA_URL/warsa_casualties_actor_units --logfile output/logs/tasks.log
