...
echo "Linking units" &&
#sed -r -i '/\s+.+:person\s+.+/d' output/casualties_processed.ttl &&
python src/tasks.py link_units output/casualties_processed.ttl output/unit_links.ttl --endpoint $WARSA_ENDPOINT_URL/sparql --arpa_unit $ARPA_URL/warsa_casualties_actor_units --logfile output/logs/tasks.log
